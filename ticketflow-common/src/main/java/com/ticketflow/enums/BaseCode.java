...
/**
 * 全局错误码枚举（每个错误码对应一个 <code, message> 对）。
 * <p>
 * 分段约定：
 * 0          成功
 * -1         系统级错误
 * 500-999    基础框架/中间件错误（UID、Sentinel、Redis Stream 等）
 * 1000-1999  用户/认证相关
 * 10000-19999 安全/加密相关（RSA、AES、签名）
 * 20000-29999 业务参数/校验（频繁提交、用户已存在等）
 * 30000-39999 商品/库存/订单
 * 50000-59999 支付/退款/对账
 */
public enum BaseCode {
    ...
    /**
     * 条件更新未命中：本次状态迁移没有抢到（状态已被另一条流程改掉）。
     * 属于并发仲裁的正常结果，不是故障——调用方需要按订单最新状态决定后续动作
     * （例如支付回调发现订单已被取消，应进入退款）。
     */
    ORDER_STATUS_CHANGED(40036, "订单状态已被其他流程变更"),

    /**
     * 缓存重建时等锁超时：并发加载同一份缓存的另一个请求卡住了。
     * 拿不到锁就不再无锁重建（那会把数据库也拖下水），直接快速失败让调用方重试。
     */
    CACHE_LOAD_LOCK_TIMEOUT(40037, "系统繁忙，请稍后重试"),
