...
public class ProgramOrderService {
    ...
    /**
     * V1 同步创建：直接操作 Redis Lua 扣减库存 → 调用 order-service 创建订单
     * V2 异步预热：{@link #createNew} 在扣减前预热缓存，减少毛刺
     * V3 异步解耦：{@link #createNew} 依然同步扣 Lua，但加 Kafka 缓冲写
     * V4 全异步：{@link #createNewAsync} 扣 Lua + 发 Kafka 后立即返回，由 consumer 建单
     *
     * 各版本共存的原因：高并发场景下逐步放量验证，
     * 新版本上线后老版本保留给非核心渠道降级使用。
     */
        ...
        if (CollectionUtil.isNotEmpty(seatDtoList)) {
            ...
            for (Entry<Long, Long> entry : seatTicketCategoryDtoCount.entrySet()) {
                ...
                Long remainNumber = Optional.ofNullable(ticketCategoryRemainNumber.get(String.valueOf(ticketCategoryId)))
                        .orElseThrow(() -> new TicketFlowFrameException(BaseCode.TICKET_CATEGORY_NOT_EXIST_V2));
