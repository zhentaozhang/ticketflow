...
/**
 * Gateway 响应出口过滤器（order=-2，与 RequestValidationFilter 同级）。
 * 拦截响应 body，按客户端渠道公钥对 data 字段做 RSA 加密（encrypt=V2 时触发）。
 * 参考 {@link org.springframework.cloud.gateway.filter.factory.rewrite.ModifyResponseBodyGatewayFilterFactory}
 * <p>
 * 加密条件：header 中 noVerify != VERIFY_VALUE 且 encrypt == V2 且 body 非空
 * 加密对象：apiResponse.data（仅对 data 字段加密，code/msg 不加密）
 * 实现方式：通过 ServerHttpResponseDecorator 包装 writeWith，在写入前改写 body
 */
@Component
@Slf4j
public class ResponseValidationFilter implements GlobalFilter, Ordered {
    ...
    // 包装 ServerHttpResponse：拦截 writeWith，在写入前篡改 body
        ...
        if ((!VERIFY_VALUE.equals(noVerify)) && V2.equals(encrypt) && StringUtil.isNotEmpty(responseBody)) {
            ...
            if (data != null) {
                try {
                    ...
                    log.error("response encrypt error, fallback to plaintext", e);
                }
            }
        }
