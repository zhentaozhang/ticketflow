...
/**
 * 支付宝支付策略实现——对接支付宝电脑网站支付。
 *
 * 使用 alipay-sdk-java，支持：
 *   pay()       → AlipayTradePagePayRequest（页面跳转）
 *   refund()    → AlipayTradeRefundRequest
 *   tradeCheck() → AlipayTradeQueryRequest
 *
 * 异步通知在 PayService 中校验 AlipaySignature.rsaCheckV1
 */
@Slf4j
@AllArgsConstructor
