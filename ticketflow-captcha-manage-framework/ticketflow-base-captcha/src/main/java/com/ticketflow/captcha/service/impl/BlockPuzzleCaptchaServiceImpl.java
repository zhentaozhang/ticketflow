...
import com.ticketflow.captcha.model.common.CaptchaTypeEnum;
...
import com.ticketflow.captcha.model.vo.PointVO;
import com.ticketflow.captcha.util.AesUtil;
import com.ticketflow.captcha.util.ImageUtils;
import com.ticketflow.captcha.util.JsonUtil;
import com.ticketflow.captcha.util.RandomUtils;
...
import static com.ticketflow.captcha.service.impl.CaptchaConstant.FIVE;
import static com.ticketflow.captcha.service.impl.CaptchaConstant.THREE;
import static com.ticketflow.captcha.service.impl.CaptchaConstant.TWO;

/**
 * 滑块验证码实现。生成拼图滑块并校验用户拖动轨迹。
 * 用户需将滑块拖动到目标位置以完成验证。
 **/
...
public class BlockPuzzleCaptchaServiceImpl extends AbstractCaptchaService {
    ...
    public ResponseModel check(CaptchaVO captchaVO) {
        ...
        cacheService.set(secondKey, captchaVO.getToken(), EXPIRE_SIN_THREE);
    ...
    public CaptchaVO pictureTemplatesCut(BufferedImage originalImage, BufferedImage jigsawImage, String jigsawImageBase64) {
        try {
            ...
            cacheService.set(codeKey, JsonUtil.toJsonString(point), EXPIRE_SIN_SECONDS);
            logger.debug("token：{},point:{}", dataVO.getToken(), JsonUtil.toJsonString(point));
            return dataVO;
        } catch (Exception e) {
            logger.error("pictureTemplatesCut error", e);
            return null;
        }
    }

    private void extracted(final BufferedImage originalImage, final String jigsawImageBase64, final int originalWidth, final int jigsawWidth, final int x) {
     ...
     * @return PointVO
     */
    private PointVO generateJigsawPoint(int originalWidth, int originalHeight, int jigsawWidth, int jigsawHeight) {
