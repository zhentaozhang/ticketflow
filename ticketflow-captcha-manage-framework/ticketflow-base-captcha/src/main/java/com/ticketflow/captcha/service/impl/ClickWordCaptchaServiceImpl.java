...
/**
 * 点选文字验证码实现。生成文字验证码图片，用户需按顺序点击指定文字以完成验证。
 **/
@Component("clickWord")
public class ClickWordCaptchaServiceImpl extends AbstractCaptchaService {
    ...
    public void initClickWordFont() {
        try {
            ...
            logger.warn("load click word font fail: {}, use default font", clickWordFontStr);
            this.clickWordFont = new Font(Font.SANS_SERIF, Font.BOLD, HAN_ZI_SIZE);
        }
    }
     ...
     * 点选文字 字体总个数
     */
    @Value("${aj.captcha.word-count:4}")
    private int wordTotalCount = 4;
    /**
     * 点选文字 字体颜色是否随机
     */
    @Value("${aj.captcha.font-color-random:true}")
    ...
    private CaptchaVO getImageData(BufferedImage backgroundImage) {
        ...
        cacheService.set(codeKey, JsonUtil.toJsonString(pointList), EXPIRE_SIN_SECONDS);
