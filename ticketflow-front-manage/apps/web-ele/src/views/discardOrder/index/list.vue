    ...
    schema: [
      {
        ...
        componentProps: {
          readonly: true,
          placeholder: '👉 请点击右侧“选择”选择具体的节目，以查看该节目产生的废弃订单',
    ...
    proxyConfig: {
      ...
      ajax: {
        query: async ({ page }, formValues) => {
          ...
          const res: any = await discardOrderPageQueryApi({
...
onMounted(async () => {
  ...
  try {
    ...
    if (list.length > 0) {
      const firstProgram = list[0];
      const form = (gridApi as any)?.formApi;
      form?.setFieldValue?.('programId', String(firstProgram.id));
      form?.setFieldValue?.('programTitle', String(firstProgram.title));
      await nextTick();
...
<template>
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('ticketflow.index.discardOrderListTitle')">
